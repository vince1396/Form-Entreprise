...
function lastDisplay($id){
    ...
    global $bdd;
    $req = $bdd->prepare("SELECT * FROM user u, fiche f, client c, projet p WHERE u.id_u =".$_SESSION['id']." AND f.id_u = u.id_u AND f.id_f = c.id_f AND c.id_c = p.id_c  AND f.id_f =".$id." LIMIT 1");
    $req->execute();
